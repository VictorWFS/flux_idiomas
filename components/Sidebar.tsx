...
export default function Sidebar() {
    ...
    return (
      ...
      <button
        ...
        className="absolute left-4 top-4 z-40 rounded-lg border border-white/10 bg-white/5 p-2 text-white/70 transition-colors hover:bg-white/10 hover:text-white lg:hidden"
      ...
      `}>
        ...
        <nav className="space-y-2 p-4 pt-8">
          ...
          {/* 3. MATERIAL DE AULA (Novo item para alunos de professores) */}
          <Link 
            href="/materials" 
            onClick={() => setIsOpen(false)}
            className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-colors border ${
          ...
          >
            {/* Ícone de um livro/documento aberto */}
            <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
