...
import TeacherDecksList from "@/components/TeacherDeckList"; // 💡 Garanta que o nome do arquivo bate com o do seu componente
  ...
  return (
    <section className="mb-12 pt-14 lg:pt-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mx-2">
        ...
        <div className="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 flex flex-col h-[400px] transition-colors shadow-lg">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-300 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full transition-colors">
            {mockAulas.map((aula) => (
              <Link 
                href={`/materials/${aula.id}`} 
                key={aula.id} 
                className="group flex items-center gap-4 rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 p-4 transition-colors hover:bg-slate-100 dark:hover:bg-white/10 cursor-pointer"
              >
                <div className="flex h-3 w-3 shrink-0 items-center justify-center">
                  {aula.isNew ? (
                    ...
                    <span className="h-2 w-2 rounded-full bg-slate-300 dark:bg-white/20 transition-colors" />
                  )}
                </div>
                <div className="flex-1">
                  <p className={`text-sm ${aula.isNew ? 'font-bold text-slate-900 dark:text-white' : 'font-medium text-slate-600 dark:text-white/70'} transition-colors`}>
        ...
        <div className="rounded-3xl border border-blue-200 dark:border-blue-500/20 bg-blue-50/50 dark:bg-transparent dark:bg-gradient-to-br dark:from-blue-900/20 dark:to-slate-900 p-6 flex flex-col h-[400px] transition-colors shadow-lg">
          <div className="flex items-center gap-3 mb-6">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/30 transition-colors">
            ...
            <div>
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 transition-colors">Atividades do professor</p>
            </div>
          </div>

          {/* O componente da lista importado */}
